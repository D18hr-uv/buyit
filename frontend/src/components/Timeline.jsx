...
const NODE_META = {
  ...
  gather_context: { icon: "🔎", color: "bg-blue-500" },
  analyze: { icon: "🧮", color: "bg-indigo-500" },
  decide: { icon: "⚖️", color: "bg-[#f0503c]" },
      ...
      <div className="mt-2 space-y-2 text-xs">
        <div className="flex flex-wrap gap-1">
          {(data.tool_calls || []).map((t) => (
            <span key={t} className="rounded bg-blue-50 px-1.5 py-0.5 font-mono text-blue-700">
              {t}()
            </span>
          ))}
        </div>
